...
const computedFields: ComputedFields = {
  ...
  dateModified: {
    type: "date",
    resolve: (doc: any) =>
      doc.date ?? fs.statSync("content/" + doc._raw.sourceFilePath).mtime,
  },
  structuredData: {
    type: "json",
    resolve: (doc: any) =>
    ...
    if (node.__rawString__) {
      if (node.tagName !== "pre") {
        ...
        if (pre.properties?.["__rawString__"]?.startsWith("npm install")) {
          ...
          pre.properties["__yarnCommand__"] = npmCommand.replace(
            "npm install",
            "yarn add"
          );
          pre.properties["__pnpmCommand__"] = npmCommand.replace(
            "npm install",
            "pnpm add"
          );
          pre.properties["__bunCommand__"] = npmCommand.replace(
            "npm install",
            "bun add"
          );
        }
